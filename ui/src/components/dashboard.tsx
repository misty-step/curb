import { Activity, AlertTriangle, CheckCircle2, Save } from "lucide-react";
...
export function PolicyStrip({ config, snapshot }: { config: ConfigView; snapshot: Snapshot }) {
      ...
      <div className="operator-list" aria-label="Current agent runs">
        {model.activeSessionRows.length > 0 ? (
          ...
          <div className="operator-empty">
            ...
            <span>{model.aliveAgents.length > 0 ? aliveAgentSummary(model.aliveAgents) : "No watched worker processes are alive."}</span>
          </div>
        )}
      </div>
    </section>
...
export function AgentTable({ agents }: { agents: AgentView[] }) {
  return (
...
function TurnTimelineRow({
  ...
  turn: TurnView;
  scale: number;
  warnPosition: CSSProperties;
  stopPosition: CSSProperties;
  selected: boolean;
  onSelect: () => void;
}) {
