...
export function App() {
  const [settings, setSettings] = useState<ApiSettings>({
    baseUrl: localStorage.getItem("curb.baseUrl") ?? SAME_ORIGIN,
    token: localStorage.getItem("curb.token") ?? "",
  });
  ...
  const [connection, setConnection] = useState<"demo" | "live" | "error">(settings.baseUrl ? "live" : "demo");
  ...
  const refresh = useCallback(
    async (next = settings, forceRescan = false) => {
      try {
        ...
        const [nextConfig, nextNotifications] = await Promise.all([
          fetchConfig(next),
          fetchNotificationHealth(next).catch<NotificationView>(() => ({
            enabled: false,
            available: false,
            status: "error",
  ...
  const model = useMemo(() => selectDashboard(snapshot), [snapshot]);

  async function persist(update: ConfigUpdate) {
    try {
      const saved = await saveConfig(settings, update);
      setConfig(saved);
      setSettingsMsg(settings.baseUrl ? "Saved." : "Demo only — run curb app to save.");
      await refresh();
    } catch (caught) {
  ...
  function connect() {
    const next = { baseUrl: settings.baseUrl || SAME_ORIGIN, token: tokenDraft };
    localStorage.setItem("curb.token", next.token);
    setSettings(next);
    void refresh(next);
  }
    ...
    <main className="shell">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button type="button" className="icon-btn" aria-label="Rescan now" onClick={() => void refresh(settings, true)}>
            <RefreshCw size={15} />
          </button>
        </div>
      </header>
        ...
        <details className="advanced">
          ...
          <div className="advanced-body">
            <input
              ...
              type="password"
              placeholder="api.token (only for a separate client)"
              value={tokenDraft}
