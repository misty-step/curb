...
- Branch: `master`
- Commit: `090cfcf2d2540a90d4de31740ad99bf09aa0ca14`
- Merge source: `Fix long-run readiness and source recovery`
- Post-merge hosted proof: GitHub Actions run `27472830448`
- Local parity proof: `git rev-list --left-right --count master...origin/master`
  returned `0 0` at the `090cfcf2` baseline.

## Canonical Current Proof

| Claim | Canonical proof | Rerun command or live source | Notes |
|---|---|---|---|
| Full pre-merge gate | `scripts/validate.sh`; hosted run `27472830448` passed full Ubuntu, full macOS, fast feedback, Windows smoke, coverage, and dependency audit on `090cfcf2`. | `scripts/validate.sh`; `gh run view 27472830448 --json conclusion,status,url` | This is the latest behavior-bearing hosted proof before the refreshed long-sidecar packet branch. The CI workflow uses Node 24-compatible official action majors (`actions/checkout@v5`, `actions/setup-node@v6`, and `actions/upload-artifact@v6`). |
...
| Long-running headless sidecar | `evidence/dogfood/2026-06-15-long-sidecar-refresh/` | `CURB_LONG_DOGFOOD_SECONDS=7200 CURB_LONG_DOGFOOD_SNAPSHOT_SECONDS=300 bash scripts/dogfood-long-sidecar.sh evidence/dogfood/$(date +%F)-long-sidecar`; `python3 scripts/verify-long-sidecar-evidence.py evidence/dogfood/2026-06-15-long-sidecar-refresh --duration-seconds 7200` | Fresh two-hour release sidecar against `090cfcf2` with private state outside the repo, final ready HTTP 200, 25/25 periodic readiness samples at `200 ready`, all sampled live/health/overview probes HTTP 200, parser acceptance, NDJSON/path/session redaction checks, 1,441 watcher ticks against a 1,080 minimum, max RSS 53,764 KB, max sampled probe latency 4.633927s, and 16 sanitized Codex source-health error events. |
...
| Dependency audit | Hosted run `27472830448`; `scripts/check-dependency-audit.sh` | `scripts/check-dependency-audit.sh --offline`; `bash scripts/check-dependency-audit.sh --online` | Online mode is hosted and explicit local proof. Offline mode avoids making the default local gate registry-dependent. |
...
scripts/validate.sh
gh run view 27472830448 --json conclusion,status,url
```
...
- The refreshed long sidecar still found 16 sanitized Codex source-health error
  events and sampled overview latency up to 4.633927s. Treat these as operator
  recovery/performance evidence, not readiness failure.
