...
- [ ] **Phase 0 — Make the policy state machine environment-agnostic.** `UsageWatch` becomes a pure policy module operating over `&[Session]` + `&[AgentTarget]` + `&dyn Enforcer` — with no reference to `platform::Snapshot`/`Process`/`Platform` and no `use crate::service`. `grep -lE "use crate::(service|platform)" src/usagewatch.rs` returns nothing. `cargo test` green. *(This is the heart; everything else is I/O plumbing around it — pi lane.)*
