...
- [ ] **Phase 0 — Invert the engine→view dependency.** `grep -l "use crate::service" src/usagewatch.rs src/runtime.rs` returns nothing: the watcher/orchestrator no longer depend on the API-view module. Correlation/policy live in the engine; `service.rs` becomes a pure view-transformer over engine outputs. `cargo test` green.
...
**Design sketch (anchor on the existing `Platform` trait, then split it):**
- `ObservationSource` — yields spend/usage + liveness facts for governed agents (today: Codex/Claude usage files + `sysinfo` process tree; tomorrow: an Olympus-supplied feed).
- `Enforcer` — warn / acknowledge / stop, given an environment-appropriate sealed target (today: signal a PID tree; tomorrow: whatever "stop this agent" means in Olympus).
- policy `Engine` — pure, environment-agnostic: thresholds, window, grace, terminated-state, escalate-supervised. No OS facts. This is what Olympus reuses unchanged.
