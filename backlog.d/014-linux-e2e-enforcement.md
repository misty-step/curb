...
- Likely first probe: on Linux, log the captured `command`/`name` of the spawned `sh -c 'while :; do sleep 1; done # <marker>'` worker and confirm whether the marker survives in the captured argv and whether the process name is `sh`/`dash` (the matcher keys on `process_names: ["bash","sh"]`).
